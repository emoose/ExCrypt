...
{
  uint64_t qkey = *(const uint64_t*)key;
  ...
  for (uint32_t i = 0; i < input_size / 8; i++)
  {
    if (encrypt) {
      *(uint64_t*)output = (*(uint64_t*)input) ^ last_block;
      ExCryptDesEcb(state, output, output, encrypt);
      last_block = *(uint64_t*)output;
    }
    else
    {
      ExCryptDesEcb(state, input, output, encrypt);
